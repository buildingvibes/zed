...
async fn test_document_symbols(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    // Open buffer on host so the language server starts
    let _buffer_a = project_a
        .update(cx_a, |project, cx| {
            project.open_local_buffer(path!("/a/main.rs"), cx)
        })
    ...
    #[allow(deprecated)]
    let mut symbol_request_handle = fake_language_server
        .set_request_handler::<lsp::request::DocumentSymbolRequest, _, _>(move |_, _| async move {
            Ok(Some(lsp::DocumentSymbolResponse::Nested(vec![
                lsp::DocumentSymbol {
                    ...
                    tags: None,
                    deprecated: None,
                    range: lsp::Range::new(lsp::Position::new(0, 0), lsp::Position::new(2, 1)),
                    ...
                    children: Some(vec![lsp::DocumentSymbol {
                        ...
                        tags: None,
                        deprecated: None,
                        range: lsp::Range::new(lsp::Position::new(1, 4), lsp::Position::new(1, 13)),
                        selection_range: lsp::Range::new(
    ...
    // Enable LSP document symbols on host
    cx_a.update(|_, cx| {
        SettingsStore::update_global(cx, |store, cx| {
            store.update_user_settings(cx, |settings| {
    ...
    // Move cursor to trigger refresh_outline_symbols which calls fetch_document_symbols
    editor_a.update_in(cx_a, |editor, window, cx| {
        editor.change_selections(SelectionEffects::no_scroll(), window, cx, |s| {
    ...
    // Wait for the LSP request and let results propagate
    symbol_request_handle.next().await.unwrap();
    executor.advance_clock(LSP_REQUEST_DEBOUNCE_TIMEOUT);
    executor.run_until_parked();

    // Host should now have LSP symbols in breadcrumbs
    editor_a.update(cx_a, |editor, cx| {
    ...
    // Client B joins and opens the same file, also with LSP symbols enabled
    cx_b.update(|cx| {
        SettingsStore::update_global(cx, |store, cx| {
            store.update_user_settings(cx, |settings| {
                settings.project.all_languages.defaults.document_symbols =
    ...
    // Client B's editor opening triggers its own breadcrumb refresh via proto forwarding
    symbol_request_handle.next().await.unwrap();
    executor.advance_clock(LSP_REQUEST_DEBOUNCE_TIMEOUT);
    executor.run_until_parked();

    // Client B should see LSP-provided symbols forwarded through proto
    editor_b.update(cx_b, |editor, cx| {
        let breadcrumbs = editor
            .breadcrumbs(cx)
